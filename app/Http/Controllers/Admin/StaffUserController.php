...
class StaffUserController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse {$data=$this->validated($request);$data['password']=Hash::make($data['password']);$data['is_admin']=false;$data['role']='class_teacher';$data['permissions']=json_encode($data['permissions']??[]);User::create($data);return back()->with('success','Class teacher user created successfully.');}
    public function update(Request $request,User $staffUser): RedirectResponse {abort_if($staffUser->role!=='class_teacher',404);$data=$this->validated($request,$staffUser);if(!empty($data['password']))$data['password']=Hash::make($data['password']);else unset($data['password']);$data['permissions']=json_encode($data['permissions']??[]);$data['is_admin']=false;$data['role']='class_teacher';$staffUser->update($data);return back()->with('success','Class teacher user updated successfully.');}
