...
<div class="card border-0 shadow-sm mb-4"><div class="card-body"><form method="GET" action="{{ route('admin.student-id-cards.index') }}" class="row g-2"><div class="col-md-4"><label class="form-label">Campus</label><select name="branch_id" class="form-select"><option value="">All Campuses</option>@foreach($branches as $branch)<option value="{{ $branch->id }}" @selected((string)request('branch_id')===(string)$branch->id)>{{ $branch->name }}</option>@endforeach</select></div><div class="col-md-5"><label class="form-label">Search Student</label><input type="search" name="q" value="{{ $search }}" class="form-control" placeholder="Admission number or student name"></div><div class="col-md-3 d-flex align-items-end gap-2"><button class="btn btn-primary flex-fill">Search</button>@if($search !== '' || request('branch_id'))<a href="{{ route('admin.student-id-cards.index') }}" class="btn btn-outline-secondary">Clear</a>@endif</div></form></div></div>
<form method="POST" action="{{ route('admin.student-id-cards.print') }}" target="_blank">@csrf
<div class="card border-0 shadow-sm"><div class="card-body p-4">
<div class="table-responsive"><table class="table align-middle"><thead><tr><th style="width:60px">Select</th><th>Student</th><th>Admission No.</th><th>Campus</th><th>Class / Section</th><th>Roll No.</th></tr></thead><tbody>
@forelse($students as $student)<tr><td><input class="form-check-input student-check" type="checkbox" name="students[]" value="{{ $student->id }}"></td><td class="fw-semibold">{{ $student->user->name }}</td><td>{{ $student->admission_no }}</td><td>{{ $student->branch?->name ?: '—' }}</td><td>{{ $student->class_name ?: '—' }} {{ $student->section ? '/ '.$student->section : '' }}</td><td>{{ $student->roll_no ?: '—' }}</td></tr>@empty<tr><td colspan="6" class="text-center text-secondary py-4">No active students found.</td></tr>@endforelse
</tbody></table></div>
